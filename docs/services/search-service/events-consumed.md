...
| `TripUpdated` | Fare, schedule, or capacity changed on an already-indexed trip — overwrite the affected fields | Same idempotent-upsert handling as `TripPublished`; if the projection doesn't exist yet, log and discard rather than attempt a partial create (this event doesn't carry enough data to construct a new projection) — see "Ordering Edge Case" below for why this is not expected to happen |
